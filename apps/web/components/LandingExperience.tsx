...
const copy={
 ar:{nav:['كيف يعمل','المزايا','البنية'],signin:'تسجيل الدخول',open:'افتح مساحة العمل',eyebrow:'نظام تشغيل لبياناتك',title1:'ملفاتك المبعثرة.',title2:'نحوّلها إلى نظام يفهمها.',body:'ارفع Excel وCSV وJSON. ORBIT يفهم البنية، يرتب البيانات، يحلل جودتها، يربطها، ويحوّلها إلى مساحة تشغيل تقدر تسألها وتبني عليها لوحات وأتمتة.',cta:'ابدأ مساحة عملك',explore:'شوف كيف يشتغل',how:'من ملف خام إلى نظام عامل',howBody:'بدل التنقل بين جداول ونسخ مختلفة، ORBIT يبني طبقة بيانات واحدة قابلة للبحث والتحليل والتشغيل.',value:'بياناتك تتحول إلى مساحة عمل.',valueBody:'ملفات موظفين أو أصول أو صيانة أو مبيعات؟ نفس ORBIT يفهم المجال ويبني لك الأدوات المناسبة فوق البيانات بدل تبدأ نظام جديد من الصفر.',secure:'قوي من الداخل، بسيط من الخارج.',secureBody:'Authentication وSessions وWorkspace Isolation وRBAC وImports وAnalytics وAutomations وREST API وAudit Logging كلها جزء من المنصة.',footer:'بياناتك. نظامك. قرارك.'},
 en:{nav:['How it works','Capabilities','Architecture'],signin:'Sign in',open:'Open workspace',eyebrow:'PERSONAL DATA OPERATING SYSTEM',title1:'Your scattered files.',title2:'Turned into a working system.',body:'Upload Excel, CSV and JSON. ORBIT understands structure, organizes records, profiles quality, connects sources, and turns your data into an operational workspace you can query and automate.',cta:'Launch your workspace',explore:'See how it works',how:'From raw file to working system',howBody:'Stop jumping between spreadsheets and stale copies. ORBIT creates one searchable, analytical and operational data layer.',value:'Your data becomes a workspace.',valueBody:'Employees, assets, maintenance or sales: ORBIT adapts the workspace to the data instead of making you rebuild a new system for every use case.',secure:'Powerful inside. Simple outside.',secureBody:'Authentication, sessions, workspace isolation, RBAC, imports, analytics, automations, REST APIs and audit logging are built into the platform.',footer:'Your data. Your system. Your orbit.'}
};
...
export default function LandingExperience({signedIn}:{signedIn:boolean}){
 ...
 return <main className="orbitLanding" dir={lang==='ar'?'rtl':'ltr'}>
  ...
  <section className="landingHero">
   <div className="landingHeroCopy"><span className="landingKicker"><i/>{t.eyebrow}</span><h1>{t.title1}<br/><em>{t.title2}</em></h1><p>{t.body}</p><div className="landingHeroActions"><Link href={signedIn?'/dashboard':'/register'} className="landingPrimary">{t.cta}<Arrow size={17}/></Link><a href="#how">{t.explore}</a></div><div className="landingProof"><span><ShieldCheck size={13}/>Source-grounded</span><span><LockKeyhole size={13}/>Workspace isolation</span><span><Zap size={13}/>Action-ready</span></div></div>
   <div className="landingWindow"><div className="landingWindowTop"><div className="landingWindowDots"><i/><i/><i/></div><span>workspace / command-center</span><Bell size={14}/></div><div className="landingWindowBody"><aside className="landingMiniSidebar"><b>O.</b>{[Layers3,Database,Search,Workflow,Braces].map((Icon,i)=><span key={i} className={i===1?'active':''}><Icon size={15}/></span>)}</aside><div className="landingCanvas"><div className="landingCanvasHead"><div><small>{lang==='ar'?'مساحة العمل':'WORKSPACE'}</small><strong>{lang==='ar'?'مركز البيانات':'Data command center'}</strong></div><button>+ Import</button></div><div className="landingStats"><article><small>{lang==='ar'?'السجلات':'Records'}</small><b>12,840</b><span>+18.4%</span></article><article><small>{lang==='ar'?'جودة البيانات':'Data health'}</small><b>96%</b><span>Healthy</span></article><article><small>{lang==='ar'?'الأتمتة':'Automations'}</small><b>18</b><span>14 active</span></article></div><div className="landingVisualGrid"><article className="landingChart"><small>{lang==='ar'?'تدفّق البيانات':'Data flow'}</small><div className="landingBars">{chartHeights.map((h,i)=><i key={i} style={{height:`${h}%`}}/>)}</div></article><article className="landingHealth"><small>{lang==='ar'?'صحة الحقول':'Field health'}</small>{healthRows.map(([name,value])=><div className="landingHealthRow" key={name}><span>{name}</span><div className="landingHealthTrack"><div className="landingHealthFill" style={{width:`${value}%`}}/></div><b>{value}%</b></div>)}</article></div><div className="landingTable"><div className="landingTableHead"><b>{lang==='ar'?'الأصول':'Assets'}</b><span>Filter</span><span>Sort</span><span>View</span></div>{[['AS-204','Makkah','Active','42,800'],['AS-205','Jeddah','Review','18,200'],['AS-206','Riyadh','Active','31,900']].map(row=><div key={row[0]}>{row.map(cell=><span key={cell}>{cell}</span>)}</div>)}</div></div></div></div>
  </section>
