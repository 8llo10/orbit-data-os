...
const advancedNav=[
  ...
  [Braces,'للمطورين','Developer tools','/dashboard/developer','API Keys و REST API'],
...
export default function AppShell({children}:{children:React.ReactNode}){
  ...
  return <div className="shell" dir={rtl?'rtl':'ltr'}>
    <aside className="sidebar">
      ...
      <nav className="nav">{mainNav.map(([Icon,ar,en,url,hint])=><Link key={url} href={url} title={hint}><Icon size={17}/><span>{rtl?ar:en}</span></Link>)}</nav>
      <button className="advancedToggle" onClick={()=>setAdvanced(v=>!v)}>{rtl?(advanced?'إخفاء الأدوات المتقدمة':'أدوات متقدمة'):(advanced?'Hide advanced tools':'Advanced tools')}</button>
      {advanced&&<nav className="nav advancedNav">{advancedNav.map(([Icon,ar,en,url,hint])=><Link key={url} href={url} title={hint}><Icon size={17}/><span>{rtl?ar:en}</span></Link>)}</nav>}
