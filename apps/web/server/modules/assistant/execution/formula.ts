...
export async function executeFormulaAction(context:ActionExecutionContext):Promise<ActionExecutionResult>{
 ...
 await db.$transaction(async tx=>{
  await tx.collectionField.create({data:{collectionId,key,label,type:'TEXT',formula,position:collection.fields.length}});
  for(const record of collection.records){const row=record.data&&typeof record.data==='object'&&!Array.isArray(record.data)?record.data as Record<string,unknown>:{};let value:unknown=null;try{value=computeFormula(formula,row)}catch{}await tx.dataRecord.update({where:{id:record.id},data:{data:JSON.parse(JSON.stringify({...row,[key]:value}))}})}
  await tx.auditLog.create({data:{workspaceId,userId,action:'ASSISTANT_FORMULA_FIELD_CREATED',entityType:'Collection',entityId:collectionId,metadata:{assistantActionId:action.id,label,key,formula,updatedRecords:collection.records.length}}});
 });
