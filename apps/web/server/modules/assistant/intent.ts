...
const rules:{intent:AssistantIntent;patterns:RegExp[];weight:number}[]=[
 {intent:'DATA_QUALITY',weight:5,patterns:[/(جود|quality|ناقص|missing|مكرر|duplicate|شاذ|anomal|غلط|invalid|null)/i]},
 ...
 {intent:'WORKSPACE_SUMMARY',weight:6,patterns:[/(لخص|ملخص|summary|overview|وش عندي|ما الموجود|what do i have|وش الملفات|ايش الملفات|ما هي الملفات|الملفات الموجودة|وش البيانات الموجودة|ايش البيانات الموجودة)/i]},
...
export function detectIntent(text:string):AssistantIntent{
 ...
 for(const rule of rules){
  let score=0;
  for(const pattern of rule.patterns)if(pattern.test(text))score+=rule.weight;
