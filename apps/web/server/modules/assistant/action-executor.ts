...
export async function executeAssistantAction(input:{action:AssistantAction;workspaceId:string;userId:string;role:WorkspaceRole}){
 const {action,workspaceId,userId,role}=input;const request=typeof action.payload?.request==='string'?action.payload.request:'';
 ...
 if(action.type==='create_view'){
  ...
  const view=await db.savedView.create({data:{collectionId,name:typeof action.payload?.name==='string'?action.payload.name.slice(0,120):'ORBIT Generated View',type:'TABLE',config:{source:'assistant',request}}});
