...
const relationRequest=/اربط|علاق|relation|join|connect|مع بعض|بين .* و/i;
const wantsResult=/احسب|حلل|طلع|اعرض|أعرض|قارن|رتب|أعلى|اعلى|اقل|أقل|average|sum|count|compare|rank|show/i;
...
export async function orchestrateAssistant(input:AssistantRunInput):Promise<AssistantResult>{
 ...
 const explicitRelation=relationRequest.test(input.message);
 if(intent==='WORKSPACE_SUMMARY'){evidence=[...workspaceEvidence(s),...largestCollections(s)];answer=composeNaturalResponse({intent,answer:'',evidence,sources,snapshot:s});}
  ...
  const relational=explicitRelation?await analyzeAcrossRelations(effectiveMessage,s):null;
 ...
 if(intent==='RELATIONSHIPS'||explicitRelation){
  actions.push(navigateAction('فتح مخطط العلاقات','/dashboard/graph'));
  const proposal=proposeRelationshipAction(s);
  if(proposal){
   if(wantsResult.test(input.message))actions.push({id:randomUUID(),type:'execute_plan',label:'إنشاء العلاقة وإكمال الطلب',requiresConfirmation:true,payload:{steps:[proposal],request:input.message,resume:true}});
   else actions.push(proposal);
  }
 }
